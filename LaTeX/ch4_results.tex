...
The result indicates that wrapper methods has a superior classification accuracy compared to filter methods in some cases. In table \ref{table:t_test_result} the classifiers who benefits from certain wrapper methods compared to filter methods are displayed.


\input{snippets/table_t_test_results}

To conclude these results, they suggests one primarily has to carefully select the classifier to use for classification. If using NB or CART and the goal is to achieve a high classification accuracy SBS or SFS should be preferred over Chi2 and Entropy. If SVM is used SBS is preferred over Chi2 and Entropy with regards to classification rate.
